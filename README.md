...
Key findings:
- Mean test accuracy improved from `0.8267` to `0.8385`, but the paired improvement was not statistically significant across the 12 dataset-seed runs (`p = 0.205`).
...
- The clearest accuracy gain appeared on `sklearn_breast_cancer`; `sonar` showed negative accuracy effects and `glass` tied in accuracy despite better shrinkage.
